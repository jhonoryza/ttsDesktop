...
synchronized public void newWriteToSDWindowHandler(PApplet appc, GWinData data) {
  ...
  appc.textSize(10);
  appc.text("copyright \u00a9 2016 GTI", appc.width/1.3f, appc.height-5);
}
...
int c = color(255, 255, lerpColor(0, 255, .33f));
...
public void createListHalte() {
  ...
  if (optYes.isSelected()) {
    if (!in1.getText().isEmpty() && PApplet.parseInt(in1.getText()) > 0 && numberOrNot(in1.getText())) {
      myKoridor.get(slideNum-1).totalHalteGo = PApplet.parseInt(in1.getText());
      myKoridor.get(slideNum-1).totalHalteBack = PApplet.parseInt(in1.getText());
    ...
    if (!in1.getText().isEmpty() && PApplet.parseInt(in1.getText()) > 0 && numberOrNot(in1.getText())
      && !in2.getText().isEmpty() && PApplet.parseInt(in2.getText()) > 0 && numberOrNot(in2.getText())) {
      myKoridor.get(slideNum-1).totalHalteGo = PApplet.parseInt(in1.getText());
      myKoridor.get(slideNum-1).totalHalteBack = PApplet.parseInt(in2.getText());
      createNewHalteWindow();
    } else {
      JOptionPane.showMessageDialog(null, "field cannot be empty and must be number greater than zero", "Error", JOptionPane.WARNING_MESSAGE);
    }
...
public void createNewHalteWindow() {
    ...
    if (!loadSettingWizardPath.isEmpty()) {
      ...
      for (int i = 0; i < lines.length; i++) {
        ...
        for (int j=0; j<list.length; j++) {
          list[j] = trim(list[j]);
          if (i == 0 && j == 1) {
            korNum = PApplet.parseInt(list[j]);
          } else if (i == 1 && j == 1) {
            ...
            if ( i % 2 == 0 && j == 1) {
              if (!change) {
                myKoridor.get(korCount).totalHalteGo = PApplet.parseInt(list[j]);
              } else if (change) {
                myKoridor.get(korCount).totalHalteBack = PApplet.parseInt(list[j]);
              }
            } else if ( i % 2 != 0 && j == 1) {
              if (!change) {
                String[] newlist = split(list[j], ",");
...
public void resetEditHalteAndKoridor(GButton source) {
...
public void refreshEditHalteAndKoridor() {
  editHalteAndKoridorWindow.noLoop();
  int i = currentHal;
  int totalHalteGo = myKoridor.get(i).totalHalteGo;
  int totalHalteBack = myKoridor.get(i).totalHalteBack;
...
public void writeToSDCard(File selection) {
  boolean change = false;
...
public void writeVoiceToSDCard(File selection) {
  if (selection == null) {
    println("Window was closed or the user hit cancel.");
...
public void createMp3(File selection) {
  String textVoice = "";
  String pitch = voicePitch;
  String rate = voiceRate;
  String vol = voiceVolume;
...
public void fileLoadSettingWizard(File selection) {
  if (selection == null) {
    println("Window was closed or the user hit cancel.");
  } else {
    browsed = true;
...
public void buttonCreateHalteHandler(GButton source, GEvent event) {
  ...
  if (!indexHalte.isEmpty() && !namaHalte.isEmpty()) {
    if (numberOrNot(indexHalte) && PApplet.parseInt(indexHalte) > 0) {
      if (halteMode == 0) {
        if (PApplet.parseInt(indexHalte)-1 < myKoridor.get(currentHal).totalHalteGo +1) {
          clearEditKoridor();
          myKoridor.get(currentHal).totalHalteGo ++;
          myKoridor.get(currentHal).namaHalteGo.add(PApplet.parseInt(indexHalte)-1, namaHalte);
        } else {
          JOptionPane.showMessageDialog(null, "index number too high", "Error", JOptionPane.WARNING_MESSAGE);
        }
      } else if (halteMode == 1) {
        if (PApplet.parseInt(indexHalte)-1 < myKoridor.get(currentHal).totalHalteBack +1) {
          clearEditKoridor();
          myKoridor.get(currentHal).totalHalteBack ++;
          myKoridor.get(currentHal).namaHalteBack.add(PApplet.parseInt(indexHalte)-1, namaHalte);
        } else {
          JOptionPane.showMessageDialog(null, "index number too high", "Error", JOptionPane.WARNING_MESSAGE);
        }
      }      
...
public void buttonCreateKoridorHandler(GButton source, GEvent event) {
  ...
  if (!indexKoridor.isEmpty() && !namaKoridor.isEmpty()) {
    if (numberOrNot(indexKoridor) && PApplet.parseInt(indexKoridor) > 0) {
      if (PApplet.parseInt(indexKoridor)-1 < korNum +1) {        
        clearKoridor();
        korNum ++;
        myKoridor.add(PApplet.parseInt(indexKoridor)-1, new koridor());
        myKoridor.get(PApplet.parseInt(indexKoridor)-1).namaKoridor = namaKoridor;
